...
### 0d. 就地助手面板（已实现）——当前的主入口形态

用户明确的目标是"**在微信里就有个助手，不用跳转也能分析**"。为此验证了三条取内容的路：

| 路径 | 实测结论 |
| --- | --- |
| 无障碍读界面 | ❌ 微信不暴露任何内容（[PoC](verification/A11Y_READABILITY_POC.md)），**已否决** |
| 截屏 + OCR | ✅ **可用** —— 用户关闭 HyperOS「屏幕共享保护」后，帧统计确认非黑屏，真机识别成功 |
| 用户主动提供内容（粘贴 / 剪贴板） | ✅ **采用**，不依赖任何系统设置 |

因此助手同时提供**手动粘贴**与**截屏分析**两条路，且都以用户主动触发为前提，符合总纲的 Just-in-Time Context。

实现与实测见 [就地助手面板](verification/ASSISTANT_PANEL.md) 与 [面板内截屏分析](verification/PANEL_CAPTURE.md)。要点：

- `ui/panel/AssistantPanel.kt` + `ui/panel/AssistantPanelState` + `system/AssistantPanelWindow.kt`（`TYPE_APPLICATION_OVERLAY`，底部面板，高度上限取屏幕高度的 68%，下方应用仍可见）
- **悬浮球点击打开面板**，不再只是把应用拉到前台（后者是装饰功能）
- 面板内「截屏分析」入口；截屏前后**重建视图**并保留状态（复用旧 View 会导致按键全部失效）
- 截屏文字按行的水平位置**自动推断说话人**，否则分析会退化为通用模板
- 「分析」按钮**随输入状态启用/禁用**，空输入时直接说明原因
- 与应用共用同一个 `ConversationRepository`，结果与完整界面一致
- 首页「打开助手面板」按钮，不依赖悬浮窗权限

**下一步最有价值的接续**：把**划词入口**（`ACTION_PROCESS_TEXT`，已实现）与面板打通 —— 在微信里选中文字 → 用 FlowAI 分析 → **面板带着这段文字直接弹出**，省掉"切应用 + 粘贴"的动作。这才是"微信里的助手"的完整体验。
...
2. **Android 12 `HIDE_OVERLAY_WINDOWS`**：App 声明该权限后，**系统设置等界面**会隐藏其窗口上方的非系统覆盖窗口。模拟器实测：系统「设置」上方悬浮球确实被隐藏（`mForceHideNonSystemOverlayWindow=true`）。

   **⚠️ 一处需要更正的推断**：微信也声明并获得了该权限，据此我曾推论"悬浮球在微信内不可见"。**真机实测推翻了这一推论** —— 悬浮球**确实显示在微信界面之上，且点击可正常拉起应用**。所以"声明该权限"并不等于"自绘界面也会隐藏覆盖窗口"；是否能隐藏取决于具体界面（系统级安全界面会隐藏，普通应用界面未必）。
   **结论**：悬浮球可覆盖微信，但**不可依赖它在所有界面上都可见**，分享与划词入口仍需保留。
...
**同一轮 PoC 确认了微信声明 `HIDE_OVERLAY_WINDOWS`；但随后真机实测表明悬浮球仍可覆盖显示在微信之上并可正常点击**（见上条第 2 点，此处更正了当时的推论）。无障碍不可读这一点不受影响。
...
4. ~~微信是否声明 `HIDE_OVERLAY_WINDOWS`（决定悬浮球是否可见）~~ —— **已由真机实测回答**：微信声明了该权限，但悬浮球仍可覆盖显示并可点击。**残留未知**：哪些具体界面会隐藏覆盖窗口（系统设置会，微信不会）。
