...
用户明确的目标是"**在微信里就有个助手，不用跳转也能分析**"。为此尝试了三条取内容的路，结论是：

| 路径 | 实测结论 |
| --- | --- |
| 无障碍读界面 | ❌ 微信不暴露任何内容 |
| 截屏 + OCR | ❌ 用户设备启用 HyperOS **屏幕共享保护**，受保护界面截出黑帧 |
| **用户主动提供内容** | ✅ **采用** |

因此助手改为：**不读屏，只分析用户给的内容，并就地以浮层展示**。这既绕开了平台安全策略，也正好符合总纲的 Just-in-Time Context。

实现与实测见 [就地助手面板](verification/ASSISTANT_PANEL.md)。要点：

- `ui/panel/AssistantPanel.kt` + `system/AssistantPanelWindow.kt`（`TYPE_APPLICATION_OVERLAY`，底部 `WRAP_CONTENT` 面板，下方应用仍可见）
- **悬浮球点击改为打开面板**，不再只是把应用拉到前台（后者是装饰功能）
- 与应用共用同一个 `ConversationRepository`，结果与完整界面一致
- 首页新增「打开助手面板」按钮，不依赖悬浮窗权限
